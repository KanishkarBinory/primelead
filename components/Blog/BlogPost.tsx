...
const STYLES = `
  ...
  .bp-date { font-family: 'Inter', sans-serif; font-size: 13px; color: #888; }
  .bp-category {
    font-family: 'Inter', sans-serif;
    font-size: 13px; color: #149AB5;
  ...
  .bp-share {
    ...
    justify-content: center; gap: 12px; margin-bottom: 32px;
  }
  .bp-share-btn {
  ...
  .bp-hero-img {
    ...
    margin-bottom: 48px; object-fit: cover; max-height: 460px;
  }

  .bp-h3 {
    font-family: 'Work Sans', sans-serif; font-weight: 700;
    color: #292929; margin: 40px 0 14px 0; line-height: 1.3em;
  ...
  .bp-video-wrap {
    ...
    height: 0; overflow: hidden; margin: 40px 0;
  }
  ...
  .bp-quote {
    ...
    padding: 20px 28px; margin: 40px 0; background-color: #fffdf0;
  }
  .bp-quote-icon {
    font-size: 48px; line-height: 1; color: #FFC501;
    font-family: Georgia, serif; display: block; margin-bottom: 8px;
  }
  .bp-quote-text {
    font-family: 'Work Sans', sans-serif; font-weight: 600;
    color: #292929; line-height: 1.6em; margin: 0 0 12px 0;
    font-size: clamp(15px, 2vw, 18px); font-style: italic;
  }
  .bp-quote-author {
    font-family: 'Inter', sans-serif; font-size: 13px;
    color: #149AB5; font-weight: 600;
    display: flex; align-items: center; gap: 10px;
  }

  .bp-tags {
    display: flex; flex-wrap: wrap; gap: 8px;
    margin: 40px 0 0; padding-top: 28px; border-top: 1px solid #e0e0e0;
  }
  .bp-tag {
    font-family: 'Inter', sans-serif; font-size: 12px;
    color: #149AB5; font-weight: 600; text-decoration: none; transition: color 0.2s;
  }
  .bp-tag:hover { color: #0e7a90; }
  .bp-tag + .bp-tag::before { content: ", "; color: #888; font-weight: 400; }

  .bp-nav { margin-top: 32px; padding-top: 24px; border-top: 1px solid #e0e0e0; }
  .bp-nav-label {
  ...
  .bp-nav-link {
    font-family: 'Work Sans', sans-serif; font-weight: 700;
    color: #149AB5; text-decoration: none;
    font-size: clamp(14px, 1.8vw, 16px); transition: color 0.2s;
  }
  .bp-nav-link:hover { color: #0e7a90; }

  .bp-comment-section {
    max-width: 780px; margin: 60px auto 0;
    padding-top: 48px; border-top: 1px solid #e0e0e0;
  }
  .bp-comment-heading {
    font-family: 'Work Sans', sans-serif; font-weight: 800;
    color: #292929; margin: 0 0 6px 0; font-size: clamp(20px, 3vw, 26px);
  }
  .bp-comment-note {
    font-family: 'Inter', sans-serif; font-size: 13px;
    color: #888; margin: 0 0 28px 0;
  }
  ...
  @media (min-width: 640px) { .bp-form-row { grid-template-columns: 1fr 1fr 1fr; } }
  .bp-label {
    font-family: 'Inter', sans-serif; font-size: 13px;
    color: #292929; display: block; margin-bottom: 6px;
  }
  .bp-label span { color: #e00; }
  .bp-input, .bp-textarea {
    width: 100%; box-sizing: border-box; border: 1px solid #d0d0d0;
    padding: 10px 12px; font-family: 'Inter', sans-serif; font-size: 14px;
    color: #292929; outline: none; transition: border-color 0.2s;
  ...
  .bp-checkbox-row label { font-family: 'Inter', sans-serif; font-size: 13px; color: #545454; }
  .bp-submit {
    background-color: #292929; color: #ffffff;
    font-family: 'Work Sans', sans-serif; font-weight: 600; font-size: 14px;
    padding: 14px 32px; border: none; cursor: pointer;
    transition: background-color 0.2s; letter-spacing: 0.02em;
  }
  .bp-submit:hover { background-color: #149AB5; }

  .bp-related { max-width: 1000px; margin: 80px auto 0; }
  @media (min-width: 1440px) { .bp-related { max-width: 1200px; } }
  .bp-related-grid { display: grid; grid-template-columns: 1fr; gap: 32px; }
  ...
  .bp-rel-card {
    ...
    color: inherit; transition: background-color 0.25s ease;
  }
  .bp-rel-card:hover { background-color: #FFC501; }
  .bp-rel-img-wrap { width: 100%; height: 180px; overflow: hidden; flex-shrink: 0; }
  ...
  .bp-rel-title {
    font-family: 'Work Sans', sans-serif; font-weight: 700; color: #292929;
    margin: 0 0 8px 0; line-height: 1.35em; font-size: clamp(14px, 1.6vw, 17px);
    display: -webkit-box; -webkit-line-clamp: 3;
    -webkit-box-orient: vertical; overflow: hidden;
  }
  .bp-rel-date { font-family: 'Inter', sans-serif; font-size: 12px; color: #888; margin: 0 0 8px 0; }
  .bp-rel-card:hover .bp-rel-date { color: #5a4a00; }
  .bp-rel-excerpt {
    font-family: 'Inter', sans-serif; font-size: 13px; color: #545454;
...
export default function BlogPost({ post }: Props) {
  ...
  // ── Safely extract just the video ID, stripping any ?si= or other params ──
  // e.g. "CdRb8eX2Yns?si=CIe33IBlZMGDRHjZ" → "CdRb8eX2Yns"
  // e.g. "CdRb8eX2Yns" → "CdRb8eX2Yns"  (already clean)
  const cleanYoutubeId = post.youtubeId
    ? post.youtubeId.split("?")[0].trim()
    : null;
      ...
      <div className="bp-page">
        ...
        <article className="bp-article">
          ...
          <div className="bp-share">
            <a
              ...
              className="bp-share-btn bp-share-fb" aria-label="Share on Facebook"
            >
            ...
            <a
              ...
              target="_blank" rel="noopener noreferrer"
              className="bp-share-btn bp-share-pin" aria-label="Share on Pinterest"
            >
            ...
            <a
              ...
              target="_blank" rel="noopener noreferrer"
              className="bp-share-btn bp-share-tw" aria-label="Share on Twitter"
            >
              <FaTwitter size={14} color="white" />
            </a>
          </div>

          <img src={post.image} alt={post.imageAlt} className="bp-hero-img" />
          ...
          {post.sections.map((section, si) => (
            <div key={si}>
              ...
              {/* YouTube embed after section index 2 */}
              {cleanYoutubeId && si === 2 && (
              ...
              {/* Pull-quote after section index 3 */}
              {post.quote && si === 3 && (
                  ...
                  <span className="bp-quote-author">
                    ...
                    <a
                      href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(
                        post.quote.text + " — " + post.quote.author
                      )}`}
                      target="_blank" rel="noopener noreferrer"
        ...
        <div className="bp-comment-section">
          ...
          <div>
            <div style={{ marginBottom: "16px" }}>
              <label className="bp-label" htmlFor="comment">Comment <span>*</span></label>
        ...
        <div className="bp-related">
          <div className="bp-related-grid">
            {related.map((rel) => (
              <Link key={rel.slug} href={`/blog/${rel.slug}`} className="bp-rel-card">
