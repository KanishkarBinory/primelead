...
import { SlideInImage } from "../animation/SlideInImage";
...
export default function StudentLife() {
  return (
    <section className="w-full overflow-hidden" style={{ minHeight: "650px" }}>
      {/* ── Mobile layout (< md) ── */}
      ...
      {/* ── Desktop layout (md+) ── */}
      <div className="hidden md:flex flex-row" style={{ minHeight: "650px" }}>
        {/* Col 1: Teal */}
        {/* Col 1: Teal */}
        <div
        ...
        >
          {/* ✅ Keep px-65 on xl, scale down only for md/lg */}
          <div className="relative z-10 px-8 md:px-10 lg:px-16 xl:px-65 pt-20">
            <h2
              className="text-white font-black leading-tight"
        ...
        >
          <div
            className="flex flex-col justify-center px-6 lg:px-10 gap-6 lg:gap-8"
          ...
          >
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="flex items-center gap-2 lg:gap-3 text-white font-semibold hover:opacity-70 transition-opacity group"
          ...
          <div
            className="relative flex items-center px-6 lg:px-10 py-8 overflow-hidden"
